...
jni::Local<jni::Object<JsonElement>> JsonElement::New(jni::JNIEnv& env, const mbgl::Value& value) {
    ...
    return value.match(
        ...
        [&] (const std::string& value1) {
            return primitive.New(env, stringConstructor, jni::Make<jni::String>(env, value1));
        },
        [&] (const double value2) {
            return primitive.New(env, numberConstructor, jni::Box(env, value2));
        },
        [&] (const int64_t value3) {
            return primitive.New(env, numberConstructor, jni::Box(env, value3));
        },
        [&] (const uint64_t value4) {
            return primitive.New(env, numberConstructor, jni::Box(env, int64_t(value4))); // TODO: should use BigInteger
        },
        [&] (const bool value5) {
            return primitive.New(env, booleanConstructor, jni::Box(env, value5 ? jni::jni_true : jni::jni_false));
        },
