...
namespace {
    ...
    const std::string geomapProtocol = "geomap://";
...
namespace mapbox {
    ...
    // TODO Mapbox 변경
    bool isGeosURL(const std::string &url) {
        return std::equal(geomapProtocol.begin(), geomapProtocol.end(), url.begin());
    }

    bool isMapboxURL(const std::string& url) {
        return url.compare(0, protocolLength, protocol) == 0 || isGeosURL(url);
    }
...
std::string normalizeSourceURL(const std::string& baseURL,
    ...
    // TODO Mapbox 변경
    const auto tpl = isGeosURL(str) ? baseURL + "/{domain}{directory}{filename}.json?access_token=" + accessToken + "&secure"
                                    : baseURL + "/v4/{domain}.json?access_token=" + accessToken + "&secure";
...
std::string normalizeStyleURL(const std::string& baseURL,
    ...
    // TODO Mapbox 변경
    const auto tpl = isGeosURL(str) ? baseURL + "/styles/{path}?access_token=" + accessToken : baseURL + "/styles/v1{path}?access_token=" + accessToken;
...
std::string normalizeSpriteURL(const std::string& baseURL,
    ...
    // TODO Mapbox 변경
    const auto tpl = isGeosURL(str) ? baseURL + "/sprites/{directory}{filename}/sprite{extension}?access_token=" + accessToken
...
std::string normalizeGlyphsURL(const std::string& baseURL,
    ...
    // TODO Mapbox 변경
    bool isGlyphs = equals(str, url.domain, "glyphs") || equals(str, url.domain, "fonts");
    if (!isGlyphs) {
        Log::Error(Event::ParseStyle, "Invalid glyph URL");
        return str;
    }
    ...
    // TODO Mapbox 변경
    const auto tpl = isGeosURL(str) ? baseURL + "/{domain}/{path}?access_token=" + accessToken : baseURL + "/fonts/v1{path}?access_token=" + accessToken;
...
std::string normalizeTileURL(const std::string& baseURL,
    ...
    // TODO Mapbox 변경
    const auto tpl = isGeosURL(str) ? baseURL + "/data{path}?access_token=" + accessToken : baseURL + "/v4{path}?access_token=" + accessToken;
...
canonicalizeTileURL(const std::string& str, const style::SourceType type, const uint16_t tileSize) {
    ...
    // TODO Mapbox 변경
    std::string result = isGeosURL(str) ? "geomap://tiles/" : "mapbox://tiles/";
