...
class AnnotationManager {
  ...
  private static class MarkerHitResolver {
    ...
    private void resolveForMarker(@NonNull MarkerHit markerHit, Marker marker) {
      ...
      // ----- TODO Mapbox 변경 - 마커 터치가 잘 안되는 문제로 터치 판단 관련 재설정 - 모든 단말을 조사할 수 없어 애매함 -----
      // 몇개 단말로 조사한 결과 bitmap.getWidth()와 bitmap.getHeight()가 실제 터치되야할 영역의 50 ~ 75%로 작게 값을 리턴하는 경우가 있음
      // bitmap.getDensity()가 480(pixelRatio = 3.0)이나 408(pixelRatio = 2.55) 인 경우는 문제가 없었으나 240(pixelRatio = 1.5)인 경우가 문제
      float weight = 1.0f;                                  //weight를 추가하여
      float pixelRatio = (float) bitmap.getDensity() / DisplayMetrics.DENSITY_DEFAULT;
      if(pixelRatio <= 2.5) weight = 2.5f / pixelRatio;     //pixelRatio = 2.55인 경우까지는 정상인걸로 확인했으니 그 이하인 경우 weight를 1보다 크게 설정

      bitmapHeight = (int) (bitmap.getHeight() * weight);
      if (bitmapHeight < minimalTouchSize) {
        bitmapHeight = minimalTouchSize;
      }

      bitmapWidth = (int) (bitmap.getWidth() * weight);
