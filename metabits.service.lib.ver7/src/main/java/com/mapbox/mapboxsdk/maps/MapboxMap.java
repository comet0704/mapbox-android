...
// TODO Mapbox 변경 - 일단 6에서 사용되던 부분이라 7에도 넣어서 에러가 안나게 함
...
public final class MapboxMap {
  ...
  // TODO Mapbox 변경 - 일단 6에서 사용되던 부분이라 7에도 추가...
  private OnMapInitializeListener onMapInitializeListener;
  ...
  // TODO Mapbox 변경 - 일단 6에서 사용되던 부분이라 7에도...  OnMapInitializeListener 추가
  MapboxMap(NativeMap map, Transform transform, UiSettings ui, Projection projection,
            OnGesturesManagerInteractionListener listener, CameraChangeDispatcher cameraChangeDispatcher,
            OnMapInitializeListener initializeListener) {
    this(map, transform, ui, projection, listener, cameraChangeDispatcher);
    onMapInitializeListener = initializeListener;
  }
  ...
  // TODO Mapbox 변경 - 일단 6에서 사용되던 부분이라 7에도...  OnMapInitializeListener 추가
  void initialise(@NonNull Context context, @NonNull MapboxMapOptions options, @NonNull OnMapInitializeListener onMapInitializeListener) {
    this.onMapInitializeListener = onMapInitializeListener;
  ...
  void onStart() {
    ...
    // TODO Mapbox 변경 - onMapInitializeListener 부분 추가
    if (onMapInitializeListener != null) {
      onMapInitializeListener.onMapInitialize(this);
    }
  ...
  public void setStyle(@Style.StyleUrl String style, final Style.OnStyleLoaded callback) {
    Log.d("MapboxMap_debug", "setStyle style = " + style + ", callback = " + callback);
  ...
  public void setStyle(Style.Builder builder, final Style.OnStyleLoaded callback) {
    ...
    Log.d("MapboxMap_debug", "setStyle style class = " + style + ", callback = " + callback);
  ...
  //-------------------------------------------------------------------------
  // TODO Mapbox 변경 - 아래 메서드들을 추가
  // 일부 함수들이 Style 클래스로 이동되었는데 style이 로딩되는 중이면 error가 발생하기 때문에 체크가 필요...
  //-------------------------------------------------------------------------
  public interface OnMapInitializeListener {
    void onMapInitialize(MapboxMap mapboxMap);
  }

  public double getTilt() {
    return transform.getTilt();
  }
  public double getZoom() {
    return transform.getRawZoom();
  }
  public void setBearing(final double azimuth) {
    transform.setBearing(azimuth);
  }
  ...
  public void setLatLng(LatLng latLng, double cx, double cy) {

  }
  public void setStyleUrl(@Style.StyleUrl String style) {
    Log.d("MapboxMap_debug", "setStyleUrl style = " + style);
    this.setStyle(style);
  }

  public void notifyStyleAlreadyLoaded() {
    if (styleLoadedCallback != null) {
      styleLoadedCallback.onStyleLoaded(style);
    }
  }

  public void setStyleUrl(@Style.StyleUrl String style, Style.OnStyleLoaded callback) {
    this.setStyle(style, callback);
  }

  public Layer getLayer(String layerId) {
    if (getStyle() != null) {
      return getStyle().getLayer(layerId);
    } else {
      return null;
    }
  }
  ...
  public void addLayer(@NonNull final Layer layer) {
    if (getStyle() == null){    //아직 스타일이 로딩되지 않은 상태이면
      getStyle(new Style.OnStyleLoaded() {
        @Override
        public void onStyleLoaded(Style style) {   //스타일이 로딩된 후에 추가

          if (style != null) {
            Layer layer1 =  style.getLayer(layer.getId());
            if (layer1 != null) { //추가하려는 layer가 이미 있으면 그냥 리턴
              Log.d("MapboxMap_debug", "addLayer " + layer.getId() + " already exists");
              return;
            }
  ...
  public void addLayerBelow(@NonNull final Layer layer, @NonNull final String below) {
    if (getStyle() == null){        //아직 스타일이 로딩되지 않은 상태이면
      getStyle(new Style.OnStyleLoaded() {
        @Override
        public void onStyleLoaded(Style style) {    //스타일이 로딩된 후에 추가
      ...
      getStyle().addLayerBelow(layer, below);
    }
  }

  public void addLayerAbove(@NonNull final Layer layer, @NonNull final String above) {
    if (getStyle() == null){        //아직 스타일이 로딩되지 않은 상태이면
      getStyle(new Style.OnStyleLoaded() {
        @Override
        public void onStyleLoaded(Style style) {    //스타일이 로딩된 후에 추가
      ...
      getStyle().addLayerAbove(layer, above);
    }
  }

  public void removeLayer(String layerId) {
    if (getStyle() != null) {
      getStyle().removeLayer(layerId);
    }
  }

  public void removeLayer(Layer layer) {
    if (getStyle() != null) {
      getStyle().removeLayer(layer);
    }
  }

  public Source getSource(String source) {
    if (getStyle() != null) {
      return getStyle().getSource(source);
    } else {
      return null;
    }
  }
    ...
    if (getStyle() == null){        //아직 스타일이 로딩되지 않은 상태이면
      getStyle(new Style.OnStyleLoaded() {
        ...
        public void onStyleLoaded(Style style) {    //스타일이 로딩된 후에 추가
          ...
          for(Source source1:sources){    //추가하려는 source가 이미 있으면 그냥 리턴
            if (source1.getId().equals(source.getId())) {
              //Log.d("MapboxMap_debug", "addSource " + source.getId() + " already exists");
              return;
            }
          }
          if (style != null) {
    ...
    if (getStyle() == null){
      getStyle(new Style.OnStyleLoaded() {
        ...
        public void onStyleLoaded(Style style) {
          style.addImage(name, image, false);
        }
      });
    }else {
      getStyle().addImage(name, image, false);
    }
  }

  public void addImages(@NonNull final HashMap<String, Bitmap> images) {
    if (getStyle() == null){
      getStyle(new Style.OnStyleLoaded() {
        @Override
        public void onStyleLoaded(Style style) {
          style.addImages(images, false);
        }
      });
    }else {
      getStyle().addImages(images, false);
    }
  }

  public <T extends Source> T getSourceAs(@NonNull String sourceId) {
    if (getStyle() != null) {
      return getStyle().getSourceAs(sourceId);
    } else {
      return null;
    }
  }
