...
public class MapView extends FrameLayout implements NativeMapView.ViewCallback {
    ...
    @Nullable
    protected MapboxMap mapboxMap;    // TODO Mapbox 변경 - protected로 변경
    ...
    // TODO Mapbox 변경 - 아래 멤버변수들 추가
    private View scopeView;
    private ScopeViewClick scopeViewClick;
    protected boolean preventMapMoveEvent;    // 지도 이동 가능 여부
    protected Activity activity;              // for textView url link
    private MapboxMap.OnMapInitializeListener onMapInitializeListener;
    final String LOG_TAG = String.format(Locale.JAPAN, "[ Debug_%s ]", MapView.class.getSimpleName());
    ...
    protected void initialize(@NonNull final Context context, @NonNull final MapboxMapOptions options) {
        ...
        // TODO Mapbox 변경 - scopeView 초기화 추가
        scopeView = view.findViewById(R.id.scopeView);
    ...
    private void initialiseMap() {
        ...
        // ----- TODO Mapbox 변경 - scopeView 관련 추가 -----
        uiSettings.setScopeView(scopeView);
        if (scopeView != null) {
            scopeView.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View view) {
                    if (scopeViewClick != null) scopeViewClick.onClick(view);
                }
            });
        }
        ...
        if (savedInstanceState == null) {
            ...
            // TODO Mapbox 변경   => onMapInitializeListener를 위해 if else 문 추가
            if (onMapInitializeListener != null) {
                mapboxMap.initialise(context, mapboxMapOptions, onMapInitializeListener);
            } else {
    ...
    private void initialiseDrawingSurface(MapboxMapOptions options) {
        ...
        // TODO Mapbox 변경 - else 부분이 무조건 실행되도록 if 문 부분 주석처리
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        // TODO: Mapbox 변경 - 지도 이동 불가 일때 싱글 터치 이벤트가 들어온 경우 이벤트 발생 방지
        if (preventMapMoveEvent && event.getPointerCount() < 2) {
            return super.onTouchEvent(event);
        }
    ...
    public interface ScopeViewClick {
        void onClick(View view);
    }

    public void setScopeViewClick(ScopeViewClick scopeViewClick) {
        this.scopeViewClick = scopeViewClick;
    }

    protected void setOnMapInitializeListener(MapboxMap.OnMapInitializeListener onMapInitializeListener) {
