...
public class InfoWindow {
  ...
  public void closeInfoWindow() {       // TODO Mapbox 변경 - public으로 변경
  ...
  InfoWindow open(@NonNull MapView mapView, Marker boundMarker, @NonNull LatLng position, int offsetX, int offsetY) {
    ...
    if (view != null && mapboxMap != null) {
	 ...
	 // TODO Mapbox 변경
     if (boundMarker.getType() == MarkerType.MARKER_VICS) {
  ...
  void adaptDefaultMarker(@NonNull Marker overlayItem, MapboxMap mapboxMap, @NonNull MapView mapView) {
    ...
    // TODO Mapbox 변경 - if문 부분 추가, else 부분에 기존 소스 넣기
    if (overlayItem.getType() == MarkerType.MARKER_VICS) {

            String snippet = overlayItem.getSnippet();
            WebView snippetWebView = (WebView) view.findViewById(R.id.infowindow_description);
            if (!TextUtils.isEmpty(snippet)) {
                snippetWebView.loadData(snippet, "text/html; charset=utf-8", "UTF-8");
                snippetWebView.setVisibility(View.VISIBLE);
            } else {
                snippetWebView.setVisibility(View.GONE);
            }
  ...
  boolean isVisible() {
    ...
    // TODO Mapbox 변경 - 메서드 추가 
    public void resizeAndVisible(LatLng position, int offsetX, int offsetY) {
        MapboxMap mapboxMap = this.mapboxMap.get();
        View view = this.view.get();
        if (view != null && mapboxMap != null) {
            view.measure(View.MeasureSpec.UNSPECIFIED, View.MeasureSpec.UNSPECIFIED);
