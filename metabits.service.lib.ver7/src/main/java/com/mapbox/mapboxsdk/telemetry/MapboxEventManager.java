...
import kr.co.geosoft.metabits.BuildConfig;
 ...
 */
public class MapboxEventManager {    //지도 업데이트 : 구버전과 호환을 위해 telmetry 폴더의 파일들을 일단 추가 - 일단 MaboxAccountManager 사용부분을 주석처리함 => 문제없을시 telmetry 폴더의 파일들을 삭제하기 바람
    ...
    public void initialize(@NonNull Context context, @NonNull String accessToken) {
        ...
        try {
            ...
            if (TextUtils.equals(userAgent, BuildConfig.MAPBOX_EVENTS_USER_AGENT_BASE) && !TextUtils.isEmpty(appIdentifier)) {
                //지도 업데이트 : okhttp 3.12.0 에는 toHumanReadableAscii메서드가 없어서 일단 주석처리
            ...
            try {
                ...
                for (Hashtable<String, Object> evt : eventsClone) {
                    ...
                    if (evt.containsKey(MapboxEvent.KEY_LONGITUDE)) {
                        ...
                        if ((lon < GeometryConstants.MIN_LONGITUDE) || (lon > GeometryConstants.MAX_LONGITUDE)) {
                            lon = MathUtils.wrap(lon, GeometryConstants.MIN_LONGITUDE, GeometryConstants.MAX_LONGITUDE);
                        }
