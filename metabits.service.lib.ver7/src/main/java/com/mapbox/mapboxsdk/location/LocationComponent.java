...
import com.mapbox.mapboxcore.location.LocationEngine;
import com.mapbox.mapboxcore.location.LocationEngineCallback;
import com.mapbox.mapboxcore.location.LocationEngineProvider;
import com.mapbox.mapboxcore.location.LocationEngineRequest;
import com.mapbox.mapboxcore.location.LocationEngineResult;

import com.mapbox.mapboxcore.permissions.PermissionsManager;
...
import kr.co.geosoft.metabits.MLog;
...
public class LocationComponent {   //
  ...
  // TODO Mapbox 변경 - LocationEnglneConnector에게 위치 정보를 전달하기 위한 리스너 callback
  private LocationEngineCallback<LocationEngineResult> locationEnglneConnectorCallback;
  ...
  private void onLocationLayerStop() {
    ...
    if (locationEngine != null && currentLocationEngineListener != null) {
   ...
   */
  public void updateLocation(@Nullable final Location location, boolean fromLastLocation) {  //지도 업데이트 : public으로 변경
    if (location == null) {
      return;
  ...
  private void updateAccuracyRadius(Location location, boolean noAnimation) {
    /*if (location != null) {
      MLog.d("Debug", "updateAccuracyRadius location accuracy = " + location.getAccuracy());
    }*/
  ...
  private OnLocationStaleListener onLocationStaleListener = new OnLocationStaleListener() {
    ...
    public void onStaleStateChange(boolean isStale) {
      // TODO Mapbox 변경 - ACCURACY_LAYER는 setAccuracyLayerEnabled를 통해서만 변경하기 위해 주석처리
      //MLog.d("Debug", "onLocationStaleListener isStale = " + isStale);
  ...
  static final class CurrentLocationEngineCallback implements LocationEngineCallback<LocationEngineResult> {
    ...
    public void onSuccess(LocationEngineResult result) {
      ...
      if (component != null) {
        ...
        // TODO Mapbox 변경 - locationEnglneConnector에게 위치를 전달
        component.locationEnglneConnectorCallback.onSuccess(result);
  ...
  @SuppressLint("MissingPermission")
  public void setLocationEngineListener(LocationEngineCallback<LocationEngineResult> callback) {

    // ----- TODO Mapbox 변경 - 주석처리 후 locationEnglneConnectorCallback 설정으로 변경 -----
    /*if (locationEngine != null) {
      locationEngine.removeLocationUpdates(currentLocationEngineListener);
    }
    currentLocationEngineListener = callback;

    locationEngine.requestLocationUpdates(
            locationEngineRequest, currentLocationEngineListener, Looper.getMainLooper());*/

    locationEnglneConnectorCallback = callback;

  }

  public void removeLocationEngineListener(LocationEngineCallback<LocationEngineResult> callback) {

    // ----- TODO Mapbox 변경 - 주석처리 후 locationEnglneConnectorCallback을 null 설정으로 변경 -----
    /*f (locationEngine != null) {
      locationEngine.removeLocationUpdates(currentLocationEngineListener);
    }*/

    locationEnglneConnectorCallback = null;

  }

  public void setPadding(int[] padding) {
    applyStyle(this.options.toBuilder().padding(padding).build());
  }
  ...
  // TODO Mapbox 변경 - AccuracyLayer 표시 여부 함수 추가
  public void setAccuracyLayerEnabled(final boolean enabled) {
    if(isEnabled) {
      if(mapboxMap.getStyle() != null) {
        locationLayerController.setAccuracyLayerEnabled(enabled);
      }else{
