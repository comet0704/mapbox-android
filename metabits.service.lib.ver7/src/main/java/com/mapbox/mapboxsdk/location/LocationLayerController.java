...
final class LocationLayerController {
  ...
  void setRenderMode(@RenderMode.Mode int renderMode) {
    ...
    // ----- TODO Mapbox 변경 - RenderMode에 따른 layer별 표시 설정 수정, ACCURACY_LAYER는 RenderMode에 따라 변경하지 않고 setAccuracyLayerEnabled 함수를 통해 변경하도록 주석처리 -----
    if (!isHidden) {
      ...
      switch (renderMode) {
        ...
        case RenderMode.COMPASS:
          ...
          setLayerVisibility(BACKGROUND_LAYER, false);
          //setLayerVisibility(ACCURACY_LAYER, !isStale);
  ...
  private void refreshSource() {
    ...
    // ----- TODO Mapbox 변경 - 원래 소스를 주석 처리 후 아래와 같이 추가 -----
    /*
  ...
  // TODO Mapbox 변경 - ACCURACY_LAYER 표시 여부 설정 함수 추가
  void setAccuracyLayerEnabled(boolean enabled) {
      setLayerVisibility(ACCURACY_LAYER, enabled);
  }
